...
const VisibilityMenu = ({ controlLabel, setVisibility, menuId }) => {
  ...
  return (
      ...
      control={
        <button className="flex items-center text-[#3800B0]">
          <img
            ...
            src={
              options.find((option) => option.title === controlLabel)?.icon ||
              EarthIcon
            }
            alt=""
          />
          <span className="text-[12px] tracking-[0.01em] mx-1 text-[#3800B0] font-semibold leading-[1.3]">
            {controlLabel}
          </span>
          <IoChevronDown />
        </button>
    ...
    >
      <Menu.Label>
        Who can see your name in your congratulation card? Also defines who can
        see the congratulations on your page.
      </Menu.Label>
        ...
        {options.map((option) => (
          <Menu.Item key={option.id}>
            ...
            <label className={menuStyles.checkbox_label} htmlFor={option.id}>
              ...
              <div>
                <p className="text-[14px] leading-[1.3] tracking-[0.01em] font-semibold text-[#3800B0]">
                  {option.title}
                </p>
                <p className="text-[12px] leading-[1.6] font-regular text-[#3800B0]">
                  {option.subtitle}
                </p>
              </div>
            </label>
