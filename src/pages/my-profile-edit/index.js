...
const ProfileEdit = () => {
  ...
  const handleChangeInput = (e) => {
    ...
    const [getUserInfoProfile, setUserInfoProfile] = useState({ full_name: "", slug: "", country: "", about: "", avatar: "", phone: "", email: "", interests: []})
              ...
              <Section>
                <form onSubmit={handleUpdateInfoProfile}>
                  ...
                  <MainInputs>
                    <div className='seclect-container'>
                      ...
                      <ul  className='countries-list'>
                        {getCountryList.map((data) => (
                          <li value={data.name} onClick={getCountryId} id={data.id} className='option'>{data.name}</li>
                        ))}
                    ...
                    <input type='text' value={ moment(value).format("DD.MM.YYYY")} readOnly className='info_input'  placeholder='Date of birth' onFocus={() => setShowCalendar(true)}></input>
                    <Calendar locale='en-EN' closeCalendar={true} next2Label={false} prev2Label={false} onChange={onChange} value={value} className={showCalendar ? "" : "hide"} />
